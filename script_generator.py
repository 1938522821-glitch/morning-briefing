...
def generate_section(section_key: str, articles: list[dict], all_sections_preview: str = "",
    ...
    system = (
        ...
        "整期节目是一段连续的播客，每个板块只是其中一段，不是独立的小节目——"
        "除非现在写的是「结尾」板块，否则绝对不要在段落最后说"
        "「下期见」「我们下次再聊」「拜拜」「明天见」之类的告别语，"
        "也不要说「今天的节目就到这里」这类收尾语，写完这部分内容就直接结束，"
        "保持和下一段的连贯性。"
        "如果某个话题在最近一期节目里已经讲过，不要原样重复——"
        "如果有新进展、新角度，可以在此基础上更新或深化；"
        "如果没有新内容，就跳过这个话题，把篇幅留给今天真正新的内容。"
        "每个板块给出的目标字数只是参考——如果今天这个板块内容确实丰富，"
        "可以适当讲得更细（背景、影响、不同角度）；如果内容少，"
        "也不要为了凑字数而注水或重复啰嗦，宁可短一点。"
    )
